...
public class TestDefaultTrigger {
  ...
  private static final Duration WINDOW_DURATION = Duration.standardSeconds(10L);
  private static final Duration ALLOWED_LATENESS = Duration.standardSeconds(5L);
  private final Instant baseTime = new Instant(0);
  @Rule public TestPipeline p = TestPipeline.create();

  @Test
  public void test_default_trigger() {
    final TestStream<String> testStream =
        TestStream.create(StringUtf8Coder.of())
            .advanceWatermarkTo(baseTime)
            .addElements(timestampedValue("hello", baseTime.plus(Duration.millis(1))))
            .advanceWatermarkTo(
                baseTime.plus(WINDOW_DURATION).plus(ALLOWED_LATENESS).minus(Duration.millis(1)))
            .addElements(
                // late data
    ...
    PAssert.that(result)
        .containsInAnyOrder(
            "1970-01-01T00:00:09.999Z:KV{hello, 1}", "1970-01-01T00:00:09.999Z:KV{world, 1}");
    p.run();
  }
}
